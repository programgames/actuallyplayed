...
| 1.7.10 | Forge | 8 | **built** | niche but devoted; a build of its own |
...
RFG is the living replacement for ForgeGradle 1.2, maintained by the GT: New Horizons team and
published on their nexus rather than the plugin portal. **Version 1.4.9, not the latest**: 2.0.x
is compiled for Java 25 and fails with `Unsupported class file major version 69` under the JDK
17 that runs the build.

What the port cost, beyond renames like `theWorld` for `world`:

- **Most of the discrete events do not exist.** No `ClientChatEvent`, no `GuiScreenEvent` input
  events, and `PlayerInteractEvent` has a different shape. They are simply dropped: the polled
  device state covers all of them, which is the case §4.1 added the polling for. This port is
  the clearest return on that decision.
- **`keyTyped` and `actionPerformed` predate the `IOException`** they gained in 1.8.
- **`@Mod` has no `clientSideOnly`** on 1.7.10.
- **`Reference` is generated by the build.** ForgeGradle 2.3 has a `replace` directive for
...
> it is private. The alternatives are worse: an access transformer for one string, or digging

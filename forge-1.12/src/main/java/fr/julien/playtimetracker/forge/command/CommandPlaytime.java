package fr.julien.playtimetracker.forge.command;
...
public class CommandPlaytime extends CommandBase {
    ...
    public CommandPlaytime(PlaytimeTracker tracker, Minecraft minecraft) {
    ...
    public String getUsage(ICommandSender sender) {
        return "playtimetracker.command.usage";
    }
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
        ...
        if (!snapshot.isPresent()) {
            reply(sender, TextFormatting.GRAY, "playtimetracker.gui.noSession");
            return;
        }
    ...
    private void printSummary(ICommandSender sender, SessionSnapshot session) {
        ...
        String state = session.getState() == ActivityState.AFK
                ? TextFormatting.YELLOW + translate("playtimetracker.gui.state.afk",
                        DurationFormatter.format(session.getIdleMillis()))
                : TextFormatting.GREEN + translate("playtimetracker.gui.state.active");
        sender.sendMessage(new TextComponentString(
                TextFormatting.GRAY + translate("playtimetracker.gui.section.session") + ": " + state));

        sender.sendMessage(line("playtimetracker.gui.played",
                DurationFormatter.format(session.getActiveMillis()),
                "playtimetracker.gui.afk", DurationFormatter.format(session.getAfkMillis())));

        sender.sendMessage(new TextComponentString(
                TextFormatting.GRAY + translate("playtimetracker.gui.section.destination") + ": "
    ...
    private void handleReset(ICommandSender sender, String[] args) {
        if (args.length < 2 || !"confirm".equalsIgnoreCase(args[1])) {
            reply(sender, TextFormatting.YELLOW, "playtimetracker.command.reset.confirm");
            return;
        }
        Optional<TargetKey> reset = tracker.resetCurrentTarget();
        if (reset.isPresent()) {
            reply(sender, TextFormatting.GREEN, "playtimetracker.command.reset.done");
        }
    }

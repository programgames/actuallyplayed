...
import fr.julien.actuallyplayed.legacy.bridge.TargetIdentity;
...
public final class PlaytimeClientHandler {
    ...
    //
    // 1.7.10 offers far fewer of these than 1.12: there is no ClientChatEvent, no
    // GuiScreenEvent input event, and PlayerInteractEvent carries a different shape. All of
    // them are covered by the polled device state instead, which is exactly the case the
    // polling was added for - see PORTING.md section 4.1.
    ...
    // 1.7.10 has no ClientChatEvent. Typing is caught by the polled key state instead, which
    // is why the polling was worth adding before this port existed.
